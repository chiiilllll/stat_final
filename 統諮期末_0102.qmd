...
這裡我們使用三種模型，分別是logistic regression、randomforest以及 xgboost，並在各自模型中選取最佳的變數組合(Accuarcy指標為準)後，再對randomforest以及xgboost調參，最後透過cross-validation盡量減少過度佩飾的影響，在這三種模型之間做比較，進而評估哪一種模型性能最好。
...
model_ela <- train(Sleep.Disorder ~ Blood.Pressure + Stress.Level + 
                   ...
                   Physical.Activity.Level + BMI.Category + Quality.of.Sleep,
                   data = data, method = "glmnet", trControl = train_control)
###cross+ela
model <- train(Sleep.Disorder ~ .,
                   data = data, method = "glmnet", trControl = train_control)
coef(model$finalModel, s = model_ela$bestTune$lambda)

# 手選變數模型
model_self <- train(Sleep.Disorder ~ Occupation + Blood.Pressure + Stress.Level,
                    data = data, method = "glm", family = "binomial", 
                    trControl = train_control)
...
xgb_model <- train(
  Sleep.Disorder ~ Sleep.Duration +Stress.Level + BMI.Category + Blood.Pressure, 
  data = data,
  method = "xgbTree",
