...
continuous_vars <- c('Age','BloodPressure_Upper','BloodPressure_Lower','Sleep.Duration','Heart.Rate') 
...
ggplot(data, aes(x = Sleep.Disorder, y = Blood.Pressure, fill = Sleep.Disorder)) +
  geom_boxplot() +
  labs(title = "Sleep disorder Distribution by BloodPressure", 
       x = "Sleep.Disorder", y = "BloodPressure") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set2")
...
#heatmap
par(mfrow = c(1,1))
numeric_vars <- data %>% select(Age, Sleep.Duration)
